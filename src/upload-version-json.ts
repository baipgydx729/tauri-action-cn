/*
 * @Author: “chenbaolong”
 * @Date: 2025-11-14 15:38:49
 * @LastEditors: “chenbaolong”
 * @LastEditTime: 2025-11-14 16:58:02
 * @Description: 
 * 
 */
...
export async function uploadVersionJSON(
  ...
  console.warn('-------versionFile:', versionFile)
  ...
  console.warn('-------assets.src:', JSON.stringify(assets))
  const asset = assets.data.find((e) => e.name === versionFilename);

  console.warn('-------assets.filter:', JSON.stringify(asset))

  if (asset) {
    if (isGitea) {
      console.warn('-------isGitea:', true)
      const info = (
      ...
      console.warn('-------github.request:', JSON.stringify(data))
      ...
      const assetData = (
       ...
       console.warn('-------github.assetData:', JSON.stringify(assetData))
  ...
  console.warn('-------assets.data:', JSON.stringify(assets.data))

  const downloadUrls = new Map<string, string>();
  console.warn('-------downloadUrls:', JSON.stringify(downloadUrls))
  ...
  for (const artifact of artifacts) {
     ...
     console.warn('-------artifacts:', JSON.stringify(artifact))

    const srcAssetName = getAssetName(artifact, assetNamePattern)
    console.warn('-------srcAssetName:', srcAssetName)
    const assetName = srcAssetName
      .trim()
      .replace(/[ ()[\]{}]/g, '.')
      .replace(/\.\./g, '.')
      .normalize('NFD')
